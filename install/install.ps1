...
prompt -Msg "This script will install Jint-lzxy/nvimconfig to:"
...
if ($USE_SSH) {
	if ((Is-Latest)) {
		Safe-Execute -WithCmd { git clone --progress -b "$env:CCLONE_BRANCH" "$env:CCLONE_ATTR" 'git@github.com:Jint-lzxy/nvimconfig.git' "$env:CCDEST_DIR" }
	} else {
		...
		Safe-Execute -WithCmd { git clone --progress -b 0.7 "$env:CCLONE_ATTR" 'git@github.com:Jint-lzxy/nvimconfig.git' "$env:CCDEST_DIR" }
	}
} else {
	if ((Is-Latest)) {
		Safe-Execute -WithCmd { git clone --progress -b "$env:CCLONE_BRANCH" "$env:CCLONE_ATTR" 'https://github.com/Jint-lzxy/nvimconfig.git' "$env:CCDEST_DIR" }
	} else {
		warn -Msg "You have outdated Nvim installed (< $REQUIRED_NVIM_VERSION)."
		prompt -Msg "Automatically redirecting you to legacy version..."
		Safe-Execute -WithCmd { git clone --progress -b 0.7 "$env:CCLONE_ATTR" 'https://github.com/Jint-lzxy/nvimconfig.git' "$env:CCDEST_DIR" }
	}
}
